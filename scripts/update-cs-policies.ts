...
const POLICY_UPDATES: PolicyUpdate[] = [
  {
    category: 'message_never_received',
    content: `## 메시지가 처음부터 안 와요

메시지 수신을 위해 아래 3단계를 모두 완료해야 합니다:
1. 발송 번호를 휴대폰 연락처에 저장
2. 카카오톡에서 해당 번호로 친구 추가
3. 카카오톡으로 성함/전화번호 뒷 4자리 전송

위 절차를 완료하셨는데도 메시지가 오지 않는 경우, 카카오톡 ID를 알려주시면 담당자가 직접 확인해 드리겠습니다.`,
    ai_instruction: `메시지 미수신(처음부터 안 옴) 처리 순서:
1. 고객이 사전 체크리스트(연락처 등록, 친구 추가, 성함/뒷4자리 전송)를 모두 완료 표시했는지 확인.
2-A. 체크리스트 모두 완료된 경우: 절차 안내를 생략하고 바로 카카오톡 ID를 요청하세요. "절차를 모두 완료해 주셨는데도 메시지가 수신되지 않는 상황이시군요. 확인을 위해 고객님의 카카오톡 ID를 알려주시면 담당자가 직접 확인 후 도와드리겠습니다."
2-B. 체크리스트 미제출 또는 일부만 완료: 3단계 절차를 안내하고 완료 여부를 확인하세요.
3. 고객이 "다 했는데 안 온다"고 재문의 시: 카카오톡 설정 > 친구 > 친구 추가 허용 확인 안내 + 카카오톡 ID 요청.
4. 카카오톡 ID 제공 시: 즉시 에스컬레이션 (관리자가 직접 친구 추가 처리).`,
  },
  {
