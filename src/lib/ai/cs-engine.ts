...
## 핵심 규칙
1. 메시지 미수신 문의 시: 연락처 저장 + 카카오톡 친구 추가 + 성함/뒷4자리 전송을 완료했는지 반드시 물어봐야 합니다. 시스템으로 확인할 수 없으므로 고객에게 직접 확인을 요청하세요. 고객이 카카오톡 ID를 제공하거나, 연락처로 친구 추가가 안 된다고 하면 즉시 에스컬레이션하세요. 관리자가 직접 카카오톡 ID로 친구 추가를 처리해야 합니다.
...
11. 메시지 미수신 — 고객이 모든 절차를 완료했다고 하는데도 메시지가 안 온다면, 관리자가 발송 로그를 확인해야 합니다. 에스컬레이션하세요.
12. 발송 시간/결제/이용기간 문의 — 기본 안내(발송 시간: 오전 4시~13시, 이용기간은 구독 조회로 확인) 후에도 고객이 추가 요청이 있으면 에스컬레이션하세요.
