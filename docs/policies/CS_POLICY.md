...
**따라서 반드시 물어봐야 함** (단, 사전 체크리스트 모두 완료 시 예외 — 아래 참조)

#### 사전 체크리스트 모두 완료 시 (1차 응답 생략)
고객이 문의 등록 시 사전 체크리스트(연락처 등록, 친구 추가, 성함/뒷4자리 전송)를 **모두 완료 표시**한 경우, 이미 절차를 다 했다고 간주한다. 1차 응답(절차 안내)을 생략하고 **바로 카카오톡 ID를 요청**한다.
```
{고객명}님, 문의 주셔서 감사합니다.

절차를 모두 완료해 주셨는데도 메시지가 수신되지 않는 상황이시군요.

확인을 위해 고객님의 카카오톡 ID를 알려주시면
담당자가 직접 확인 후 도와드리겠습니다.
```
→ 카카오톡 ID 제공 시 **에스컬레이션** (3차와 동일 처리)

#### 1차 응답 (체크리스트 미제출 또는 일부만 완료 시)
```
{고객명}님, 문의 주셔서 감사합니다.
