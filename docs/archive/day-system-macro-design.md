...
```
if paused_at != null:
  active_pause = 오늘 - paused_at (현재 진행 중인 정지 일수)
else:
  active_pause = 0

current_day = (오늘 - start_date + 1) - paused_days - active_pause
```

- Day는 DB에 저장하지 않고 매번 계산한다
- `start_date` = 주문일 + 1 (주문 다음날 시작)
- `paused_days` = 과거 정지 일수 합계 (해제 시 누적)
- `active_pause` = 현재 정지 중이면 그 기간도 차감 (정지 중 Day가 올라가지 않음)
- 모든 날짜 계산은 KST 기준
...
| `failure_date` | date | 실패 발생일 (UI에서 "3/22 친구 못 찾음" 표시용) |
...
end_date = start_date + duration_days - 1 + paused_days + active_pause + 밀린일수
```
...
recovery_mode = null:
  pending_count = 1   → 정상 발송 (1개)
  pending_count = 2   → 1일 자동재시도: 밀린 것 + 오늘 같이 발송 (2개)
  pending_count >= 3  → 2일 이상 연속 미발송: 대기열 생성 안 함, 관리자 확인 필요로 전환

recovery_mode = 'bulk':
  pending_days 전부 발송 (밀린 것 + 오늘 전부, pending_count 제한 없음)

recovery_mode = 'sequential':
  last_sent_day + 1 만 발송 (하루씩, pending_count 제한 없음)
```
...
|------|--------------|---------------|----------|
| 직접 보냈어요 | current_day | null | 변동 없음 |
...
→ start_date = 시작일
→ paused_days = 0 (초기화)
→ failure_type = null (초기화)
→ recovery_mode = null (초기화)
→ end_date 자동 재계산
```
...
2. 구독별 성공/실패 집계
3. 전체 성공 구독 → last_sent_day 업데이트 + failure_type/failure_date를 null로 초기화
4. 실패 구독 → failure_type, failure_date 설정
5. 사람 단위 실패 전파 (friend_not_found 시)
6. recovery_mode 초기화 조건 확인 (섹션 3.7)
...
- recovery_mode가 null인 경우: pending_count < 3 (2일 이상 연속 미발송 아님)
- recovery_mode가 설정된 경우: pending_count 제한 없음 (bulk/sequential 복구 중)
...
recovery_mode = null:
  pending_count = 1 → 정상 발송 (1개)
  pending_count = 2 → 밀린 것 + 오늘 (2개 같이)
  pending_count >= 3 → 대기열 생성 안 함

recovery_mode = 'bulk':
  pending_days 전부 (제한 없음)

recovery_mode = 'sequential':
  last_sent_day + 1 만 (제한 없음)
```
